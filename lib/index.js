...
internals.GoodConsole.prototype._printEventJson = function (event) {
...
internals.GoodConsole.prototype._formatResponse = function (event, tags) {
    ...
    if (this._settings.json) {
      ...
      return this._printEventJson(log);
    }
    ...
    log.data = Util.format('%s: %s %s %s %s (%sms) %s', event.instance, method, event.path, query, statusCode, event.responseTime, responsePayload);
    return this._printEvent(log)
